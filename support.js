...
(() => {
  ...
  function boot(runtime, doc = document) {
    ...
    // The embedded document is complete in production. Only the editor iframe
    // needs a second fetch so streamed template updates can replace it.
  ...
  function cleanRichStyle(styleText) {
    ...
    return String(styleText || "").split(";").map((part) => {
  ...
  function sanitizeRichHtml(html) {
    ...
    const allowed = new Set(["SPAN", "STRONG", "B", "EM", "I", "U", "S", "BR", "SUB", "SUP", "P", "DIV"]);
    const clean = (parent) => {
      Array.from(parent.childNodes).forEach((node) => {
        ...
        Array.from(node.attributes).forEach((attr) => {
          if (attr.name.toLowerCase() === "style") {
            const nextStyle = cleanRichStyle(attr.value);
  ...
  function walkText(node) {
    ...
    return (vals, ctx, key) => h(
      ...
      ...parts.map((p, i) => {
        ...
        if (looksLikeRichText(v)) {
          return h("span", {
            key: i,
            className: "sc-interp tt-rich-text",
