...
for (const [key, file, route, expectedTitle] of routes) {
  ...
  if (scriptPayload) {
    ...
    assert(updateBridgesBody.includes('var elapsed=Math.max(0,performance.now()-run.startedAt), playP=this.cl(elapsed/DUR,0,1);') && updateBridgesBody.includes('prog=playP*FINAL;'), `${file}: bridge assembly is not driven by elapsed time`);
    ...
    assert(updateBridgesBody.includes('elapsed>=DUR+run.hold') && decodedScript.includes("run.phase='advancing'") && decodedScript.includes('if(p>=1 && now-run.advanceAt>=(this._reduceMotion?180:450)) this.releaseBridge();'), `${file}: bridge handoff does not retain the lock through automatic advancement`);
